...
#include "didal/distr/block_distribution.h"
...
*/
    ...
    const int rank = env.thisNode().rank();
    const int size = env.allNodes().size();
    ...
    ddl::MeshTopology<1> topology(env.thisNode(), size);

    // 1d decomposition by k.
    ddl::BlockDecomposition<1> kms_decomp({kms, size});
    ddl::BlockDistribution<1> distr(&kms_decomp, &topology);
